...
class LTL(Specification):
    ...
    def _init__atoms_formula(self, boolean_formula):
    ...
    def __translate_bool_to_ltl(self, formula: str = None) -> str:
        if isinstance(formula, str):
            ...
            ltl_formula_str = deepcopy(self._boolean_formula.to_spot())
        for key, value in self._atoms_dictionary.items():
            ltl_formula_str = ltl_formula_str.replace(
                key, value.represent(LTL.OutputStr.SIMPLIFIED)
            )
