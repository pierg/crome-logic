...
class LTL(Specification):
    ...
    def __post_init__(self):
        ...
        if self._typeset is None:
            ...
            if self._parse_env_systems:
                ...
                set_ap_e = set(
                    map(lambda x: BooleanUncontrollable(name=x), set_ap_str_e)
                )
    ...
    def typeset_complete(self) -> Typeset:
        return (
            self.typeset
            + self.refinement_rules.typeset
            + self.adjacency_and_mutex_rules.typeset
        )
    ...
    def export_to_json(self):
        json_content = {}
        if self.formula == "1":
            json_content = {"ltl_value": "true", "world_values": [[], [], []]}
        else:
            sensor = []
            location = []
            action = []
